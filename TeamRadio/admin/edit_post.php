...
if (isset($_POST['formadd'])) {
  if (!empty($_POST['blog_titre']) && !empty($_POST['blog_texte']) && !empty($_POST['blog_date'])) {
    ...
    $query = $bdd->prepare("UPDATE `blog` SET titre = ?, texte = ?, date = ? WHERE idblog = ".$_POST['edit_id']);
    if ($query->execute(array($blog_titre,$blog_texte,$blog_date))) {
...
if (isset($_POST['edit_id'])) {
  $query = $bdd->query("SELECT * FROM blog WHERE idblog = ".$_POST['edit_id']);
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="box box-success">
        ...
        <input type="text" placeholder="Titre de l'article" name="blog_titre" class="form-control" value="<?php echo htmlspecialchars_decode($data['titre']); ?>">
        <br>
        <p><b>Texte :</b></p>
          <div>
        ...
        <input type="text" name="blog_date" value="<?php echo date('m/d/Y h:i A', strtotime($data['date'])); ?>"/>
        <br>
        <br>
        <center><button type="submit" style="width:150px;" class="btn btn-block btn-primary" name="formadd">Modifier l'article</button></center>
        </form>
        <br>
...
<!-- ./wrapper -->
<div id="datavalue" style="display:none;">
<?php echo $data['texte']; ?>
</div>
<!-- jQuery 2.2.3 -->
